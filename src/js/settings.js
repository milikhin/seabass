define(['app/utils/storage', 'co'], function (storage, co) {
	...
	SettingsController.prototype._initFileTreeSettings = function () {
		...
		return co(function* () {
			...
			var savedSource = yield self.get('fileTreeSource');
      var realSource = savedSource;
      if (savedSource) {
				document.getElementById('file-tree-source').value = savedSource;
			} else {
				if (!window.LocalFileSystem) {
	...
	SettingsController.prototype._initEditorSettings = function () {
		...
		return co(function* () {
			/* Editor settngs*/
      var is = yield self.get('isCustomFontSize')
      console.log('is?', is);
