				...
				directoryReader.readEntries(function (entries) {
					...
					for (var i = 0; i < entries.length; i++) {
						...
						promises.push(co(function* () {
							var entry = entries[i];
							if (!entry.isDirectory && !window.LocalFileSystem && window.chrome) {
								var writableEntry = yield new Promise(function (resolve, reject) {
									chrome.fileSystem.getWritableEntry(entry, resolve);
								});
							} else {
								writableEntry = entry;
							}

							writableEntry.nativeURL = writableEntry.nativeURL || writableEntry.fullPath;
							var fileDescription = {
								'text': writableEntry.name,
								'id': md5(writableEntry.nativeURL),
								'entry': writableEntry,
								'itree': {
									state: {
										selectable: false
								...
								'children': writableEntry.isDirectory > 0
							};
